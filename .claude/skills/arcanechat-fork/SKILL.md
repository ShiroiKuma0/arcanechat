...
| App version scheme | `versionName <upstream>+<N>`, base `versionCode <upstreamCode> + N` (final per-ABI is `×10+abi`); currently `2.53.0+5` / base `30000751`; committed bump per feature (see Versioning) |
...
- **On an upstream rebase** (new upstream version), reset the tail: `versionName` → `<new upstream versionName>+1`, base `versionCode` → `<new upstream versionCode> + 1` (read the new upstream values from the freshly-rebased `build.gradle` first). This line conflicts on every rebase — resolve by re-deriving, not by blindly keeping ours. **Monotonic guard:** if the reset code would be ≤ the last *shipped* base code (upstream's base can advance more slowly than our tail consumed), start the tail at the first `N` that exceeds it — an upgrade must never be a downgrade. Real case: `2.53.0`'s base is `30000746` but `2.49.0+7` had shipped as `30000749`, so the `2.53.0` line started at `+4` = `30000750` (rule documented in the `build.gradle` comment). During the rebase replay, resolve *intermediate* version-bump commits to the plain reset value and land the **final** commit on the guarded value — mid-stack values are never installed.
- **Adoption point:** the scheme started at `2.49.0+1` / `30000743` (Step 9). Builds before that were pre-convention and carried the bare upstream `2.49.0` / `30000742`. The `2.53.0` line started at `2.53.0+4` / `30000750` (monotonic guard above).
