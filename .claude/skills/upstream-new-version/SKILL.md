...
- **Re-derive the version on every rebase — never blindly keep ours.** The version block in `build.gradle` `defaultConfig` conflicts on every sync. Set `shiroikumaUpstreamCode`/`shiroikumaUpstreamName` to the new upstream values (read them from the freshly-rebased `build.gradle`) and **reset `shiroikumaBuild` to `1`** — the counter always restarts at `+001` on a new upstream release (白い熊, 2026-08-12). `versionCode` is computed as `(upstreamCode − 27000000) × 100 + N` and `versionName` as `<upstream>+<NNN>`, so they cannot disagree. **The old `<upstreamCode> + N` scheme and its monotonic guard are retired** — do not reintroduce the "first N that exceeds the last shipped code" arithmetic. Resolve intermediate bump commits to the HEAD side. See the fork skill's **Versioning** section for the ceiling maths and the Groovy parse trap.
...
- **`build.gradle`** — the `applicationId "shiroikuma.arcanechat"` line (keep ours), the **version block (re-derive: new `shiroikumaUpstreamCode`/`Name`, `shiroikumaBuild = 1`** — never blindly keep ours), the **disabled per-ABI `versionCodeOverride`** in `applicationVariants` (keep ours — restoring upstream's `× 10 + abi` branches breaks the version formula), plus the fork's `buildFeatures`/`debug`/`-Xlint:-options` tweaks.
...
| Version tail | `<new>+001` / code `(<new-code> − 27000000) × 100 + 1` | `build.gradle` `defaultConfig` |
...
Stop after Step 6 with a built, signed, sideloaded `<new>+001` APK and **wait**. When 白い熊 has tested on-device and **explicitly says "Push"**, then — and only then — in that turn:
...
- **Monotonic version guard — RETIRED 2026-08-12.** It used to bite on every sync (`2.53.0` started at `+4`, `2.56.0` at `+6`, `2.58.1` at `+015`) because the old `<upstreamCode> + N` scheme had exactly **one** code slot per upstream release, while upstream's base advanced by ~1 per release and we spent a dozen-plus builds on each. 白い熊 replaced it with `(upstreamCode − 27000000) × 100 + N`, so the counter now always restarts at `+001`. If you find yourself computing "the first N that exceeds the last shipped code", you are on the old scheme — stop and read the fork skill's Versioning section.
...
- **An upstream build-system jump rewrites the block our version override lives in (v2.59.1 sync, 2026-08-26).** Upstream went **AGP 8.11.1 → 9.3.1 / Gradle 8.13 → 9.5.0** and replaced `android.applicationVariants.all { }` with `androidComponents { onVariants(selector().all()) { … } }`. Our "per-ABI `× 10 + abi` disabled" edit sits exactly there, so it is a **port, not a keep-ours**: on the new API, delete upstream's three `output.versionCode.set(vCode * 10 + …)` branches and leave `output.versionCode` alone. **Verify the result in the APK, not the source** — `aapt2 dump badging <apk> | head -1` must show the bare `defaultConfig` code (`300075301`, not `3000753012`). Two related notes: the new wrapper is an **uncached ~130 MB download**, so the never-committed `networkTimeout=600000` bump is load-bearing on such a sync; and upstream may have **adopted one of our own fixes** in the same jump (v2.59.0 took our Step 9 `buildConfig = true` / `gradle.properties` change verbatim), in which case that hunk resolves to **upstream**, not to ours — check before reflexively keeping the fork side.
- **A retranslated neighbour makes a locale conflict that is a UNION (v2.59.1 sync).** `values-gl` stopped the rebase not because of `app_name` but because upstream retranslated `ok` on the very next line. Keep our `app_name`, take upstream's translation — same shape as the `ConversationListItem` badge union.

- **Upstream rewrote the version-override block AGAIN, one release later (v2.59.2 sync, 2026-09-05).** After the AGP 9 move of v2.59.1, v2.59.2 folded the APK **file-name** suffix into the very branches the fork deletes: `def apkSuffix = "-${output.versionName.get()}.apk"` is built above them and gets `apkSuffix = "-${ABI_FILTER}" + apkSuffix` inside the first one, with `outputFileName.set(…)` moved *below*. Deleting the three `output.versionCode.set(vCode * 10 + …)` branches wholesale therefore also loses the ABI from the APK name. Correct port: keep the `ABI_FILTER` branch **for `apkSuffix` only**, drop the other two branches and the then-unused `vCode`/`abiFilter`/`abiName` locals. Treat this block as a **port on every sync**, never a keep-ours, and verify in the APK (`aapt2 dump badging` must show the bare `defaultConfig` code — `300075401` for `2.59.2+001`, not a 10× value).
- **The `build.gradle` `dependencies { }` block is a UNION conflict (v2.59.2 sync).** Our single `androidx.documentfile` line (customization 17) sits in a block upstream refreshes wholesale — that release bumped exifinterface/work/constraintlayout/Glide, hoisted `lifecycle`/`glide` versions into `def`s, dropped Guava and swapped `subsampling-scale-image-view` for its `-androidx` artifact. Keeping our side silently reverts all of it: take upstream's block and re-insert our one line.
- **`README.md` conflicts whenever upstream touches its own README (v2.59.2 sync).** Ours replaces it wholesale with the fork story, so the resolution is always `git checkout --theirs README.md` during the rebase (in a rebase, "theirs" is the commit being replayed — i.e. ours). Nothing of upstream's README is carried over.
- **An upstream `compileSdk` bump needs the platform installed locally (v2.59.2 → `compileSdk 37`).** Check `ls $ANDROID_HOME/platforms` before building; a missing platform fails Gradle configuration, not the native step. (Both `android-37.0` and `android-37.1` plus build-tools `37.0.0` were already present on this machine.)

## One-line summary of the flow

`fetch upstream --tags` → new tag? (else stop) → **OK-gated Markdown table of UI/functional improvements since our base + WAIT** → ff `main` → rebase `custom` onto `${new_tag}` (reconcile per the fork skill's conflict map; **re-derive the version block, resetting the counter to `+001`**) → `submodule update` (core moved → long native rebuild) → verify invariants → **apply arcanechat-fork to build/sign/deploy via /after-build** → 白い熊 tests → on "Push": push `main` (ff), force-with-lease `custom`.
